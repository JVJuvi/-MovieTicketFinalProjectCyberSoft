...
export default function Menu(props) {
    ...
    return (
        ...
        <div className="grid grid-cols-5">
            <div className="col-span-1 h-full p-3 space-y-2 w-60 bg-gray-100 text-coolGray-800">
                ...
                <div className="divide-y divide-coolGray-300">
                    <ul className="pt-2 pb-4 space-y-1 text-sm">
                       {mangDanhMuc.map((item,index)=>{
                           const classbg = ['flex', 'items-center', 'p-2', 'space-x-3', 'rounded-md', maDanhMuc === item.maDanhMuc ? 'bg-black text-white' : '']
                           return (
                            <li key={index} className="bg-coolGray-100 text-coolGray-900" onClick={()=>{
                                setMaDanhMuc(item.maDanhMuc)
                            }} >
                                <a href="#" className={_.join(classbg, ' ')}>
                            ...
                            {_.filter(data, item => item.maDanhMuc === maDanhMuc).map((item,index)=> {
                                return (
                                    <div key={index} class="p-4 lg:w-1/3">
                                        ...
                                        <h1 class="title-font sm:text-1xl text-xl font-medium text-gray-900 mb-3">{item.tenMonAn}</h1>
                                            <img className="mt-5" src={item.hinhAnh} alt="" />
                                        <a class="text-indigo-500 inline-flex items-center">Xem chi tiết
