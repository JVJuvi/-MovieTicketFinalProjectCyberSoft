...
export default function Fill() {
    ...
    return (
        <div>
            <Popup openModal={openModal} setOpenModal={setOpenModal} monAn={monAn} />
        <div className="grid grid-cols-6">
            <div className="h-full p-3 space-y-2 col-span-1 bg-coolGray-50 text-coolGray-800">
                <div className="flex items-center p-2 space-x-4">
                    ...
                    <div>
                        <h2 className="text-lg font-semibold">Phan Quoc Cuong</h2>
                ...
                <div className="divide-y divide-coolGray-300">
                    <ul className="pt-2 pb-4 space-y-1 text-sm">
                        {mangDanhMuc.map((danhMuc, index)=>{
                            ...
                            const classDanhMuc = ['flex', 'items-center', 'p-2', 'space-x-3', 'rounded-md', maDanhMuc === danhMuc.maDanhMuc ? 'bg-black text-white' : '']

                            return (
                                <li className="bg-coolGray-100 text-coolGray-900" key={index} onClick={()=>{
                                    setMaDanhMuc(danhMuc.maDanhMuc)
                                }}>
                                    <a href="#" className={_.join(classDanhMuc, ' ')}>
            ...
            <div className="col-span-5">
                <div className="text-center mt-10">
                    <h1 className="text-purple-300 text-7xl">MENU</h1>
                </div>
                            ...
                            {_.filter(data, item => item.maDanhMuc === maDanhMuc).map((item,index)=>{
                                return (
                                    <div class="p-4 lg:w-1/3" key={index}>
                                        <div class="h-full bg-gray-100 bg-opacity-75 px-4 pt-16 pb-24 rounded-lg overflow-hidden text-center relative">
                                            <h2 class="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">{item.tenMonAn}</h2>
                                            ...
                                            <a class="text-indigo-500 inline-flex items-center" style={{cursor: 'pointer'}} onClick={()=>{
                                                setOpenModal(true);
                                                setMonAn(item);
                                            }}>Chi tiết
                                            ...
                                            <div class="text-right absolute bottom-0 left-0 w-full py-4">
                                                <span class="text-gray-400 mr-3 inline-flex items-center leading-none text-sm pr-3 py-1 text-2xl" >
                                                    Giá: {item.gia} vnđ
                                                </span>
                                            </div>
                                        </div>
